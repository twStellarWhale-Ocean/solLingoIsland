...
> 以表格描述本層動作／SOP（orgsopcat/orgSop→teamSop→prsnSop 逐層 zoom）；編號全程上下對應。orgSop 數＝需求域數（4：螢幕／影片／筆記／電子書）＋平台維保（1）＝5，與 spec（10：螢幕／影片／筆記各 1＋電子書增量1 三條 spec#4–#6＋電子書增量2 四條 spec#7–#10）不等（維保為平台 orgSop、無獨立 value-spec；電子書一域 orgSop#5 跨兩增量對應七條增量 spec）。
...
| orgSop#5-電子書擷取學習查詢 | **桌面原生 GUI**（統一主視窗「電子書」分頁：最左常駐書目清單＋右側子頁籤【內容】逐段閱讀器／【獲得】匯入卡片，#236 頁面整併） | 選/拖 `.epub` 匯入（含批次）、解析中繼/spine/目錄/封面、書櫃列出整理（封面縮圖/主題篩選/四鍵排序/依 dc:identifier 去重/右鍵刪除清空）、藏書持久化與備份；**【內容】翻開逐段導讀閱讀（上/下一段/繼續、章節或段落跳讀、進度記憶）＋TTS 逐段朗讀＋逐字查詞加筆記＋段落說話人上色**（增量2） |
...
> need（spec#N，客戶目的／營運議題／成效期待，不混工程手段；粒度一致互不包含）＋端對端驗收課目（e2eTest，以 orgSop 為驗收單元、依 productReadme）。螢幕／影片／筆記複習各一 spec（spec#1–#3）、電子書增量1 三條 spec（spec#4–#6＝匯入／書櫃／持久化備份）、電子書增量2 四條 spec（spec#7–#10＝逐段導讀閱讀／TTS 逐段朗讀／閱讀查詞筆記／段落說話人上色），電子書兩增量同域 orgSop#5 承接；平台維保為 orgSop#4、無獨立 value-spec（其行為由各 spec 之維運面承接、e2eTest#04 驗）。
...
* **spec#7-電子書內容逐段導讀閱讀**（增量2）：能把匯入書櫃的 EPUB **翻開閱讀**——於「電子書」分頁【內容】選一本書、依章節目錄（spine 順序）讀章節，內文以**逐段**呈現、**當前段放大高亮**，可**上一段／下一段／繼續**逐段前進、**雙擊左側章節目錄或任一段落即跳讀**該處；**讀到哪一章／段跨啟動記住**（重開程式回到上次閱讀位置）。章節渲染僅**純文字內文**，且**段落（`<p>`）與清單項（`<ul>`／`<ol>` 之 `<li>`）混排之章兩者皆須成段、依原文順序交錯呈現**——對白以項目符號標記者（`<li>` 內 `Name: 台詞`）不得遺漏；純導覽用之清單項（僅含單一內部連結者，如目錄章）不成段、不朗讀。表格與複雜排版（雙欄等）**本階段未支援**——明確為 MVP 取捨、遇之以純文字呈現不當機；使用者原檔唯讀不改寫。
* **spec#8-電子書逐段 TTS 朗讀**（增量2）：能對閱讀中的電子書**逐段朗讀**——按朗讀鈕以 Windows 內建語音（離線、免金鑰）唸出**當前段**，**唸完自動前進到下一段**續讀（形成連續導讀），可**續讀／重聽本段／調整語速**（沿用既有發音／語速設定）；朗讀游標與逐段導讀同步（＝當前段）、切書／跳讀／暫停即止、不誤讀；無語音包明確降級不當機。
* **spec#9-電子書閱讀中查詞與加入筆記**（增量2）：閱讀中能於任一段落**逐字點選查詞**——點某英文單字即以同一套查詢取得**英文原文／KK 音標／繁中翻譯**並可聆聽（來源改內文文字、免辨識誤差），並可將擷取之字／句**加入同一套「我的筆記」**（去重／資料夾／發音練習）；與螢幕、影片來源之筆記無差別共存、完全共用既有查詢與筆記子系統、不另造；金鑰不落地。
* **spec#10-電子書段落說話人上色**（增量2）：對白密集之書能**依說話人辨色閱讀**——段落開頭之 `Name:` 前綴（劇本／對話體）辨識為說話人，依**使用中主題之 12 色**為該段說話人上色（無前綴段不標）；沿用影片頁之**說話人面板**（篩選只看某些說話人、於指定說話人自動暫停導讀），主題切換即重算配色。說話人一律取自內文既有前綴、免 AI。
...
* **spec#7（電子書內容逐段導讀閱讀）**：章節依 spine 順序渲染成功率、段落（`<p>`）切分正確；當前段高亮與上一段／下一段／繼續前進正確、雙擊章節目錄／段落跳讀命中；閱讀進度（章／段）跨啟動還原正確；EPUB 內嵌圖／複雜排版未支援之明確降級（純文字、不當機）；原檔唯讀不改寫稽核。
* **spec#8（電子書逐段 TTS 朗讀）**：逐段朗讀以 Windows 內建語音發聲、唸完自動前進到下一段（`SpeakCompleted` 驅動）；續讀／重聽本段／調速正確、朗讀游標與當前段同步；切書／跳讀／暫停即止不誤讀；無語音包明確降級不當機。
* **spec#9（電子書閱讀中查詞與加入筆記）**：內文逐字點選命中（標點不誤取）、查得三欄與朗讀沿用既有；加入筆記去重／歸類正確、與螢幕/影片來源筆記無差別共存；查詢降級／重試沿用既有；金鑰不落地稽核。
* **spec#10（電子書段落說話人上色）**：段首 `Name:` 前綴抽取正確（合唸拆原子、非前綴段不標）、依使用中主題 12 色上色且主題切換重算；說話人面板篩選／於指定說話人暫停正確；免 AI 稽核。
...
* **docProgTest#09-電子書內容閱讀**（orgSop#5）：開電子書分頁【內容】、選一本書、依章節目錄逐段閱讀（上一段/下一段/繼續、雙擊章節或段落跳讀）、按朗讀鈕逐段朗讀（唸完自動前進、調速/重聽）、點內文單字查詞並加入我的筆記、對含 `Name:` 前綴之書見段落依說話人上色並以面板篩選/暫停、關程式重開回到上次閱讀位置。
* **e2eTest#06-電子書內容閱讀一圈**（依 orgSop#5、docProgTest#09）：於「電子書」分頁【內容】開一本已入櫃的書 → 依左側章節目錄選章、內文逐段呈現當前段高亮 → 上一段／下一段／繼續逐段前進、雙擊某章／某段跳讀 → 按朗讀鈕逐段朗讀、唸完自動前進、調速／重聽本段 → 於某段點一英文單字查得三欄並聆聽、加入我的筆記、於筆記分頁見該筆 → 對含 `Name:` 前綴之對話書見段落依說話人上色、以說話人面板篩選／於指定說話人暫停 → 關程式重開回到上次閱讀章／段 → 全圈閉合、無內嵌圖之純文字段落不當機、查詢與筆記後段沿用既有、原檔未被改寫。
...
| | teamSop#5.4 | prsnSop#5.4.1〔prsn玩家·電子書內容頁〕開書、依章節目錄逐段閱讀（段落純文字/當前段高亮/上一段·下一段·繼續/雙擊章節或段落跳讀/閱讀進度記憶）〔[runWi自訂Usr逐段導讀閱讀]〕 |
| | teamSop#5.5 | prsnSop#5.5.1〔prsn玩家·電子書內容頁〕逐段 TTS 朗讀（喇叭唸當前段/唸完自動前進/續讀·重聽·調速）〔[runWi自訂Usr逐段朗讀]〕 |
| | teamSop#5.6 | prsnSop#5.6.1〔prsn玩家·電子書內容頁〕閱讀中逐字點選查詞並加入我的筆記（複用既有查詢/筆記）〔[runWi自訂Usr閱讀查詞加筆記]〕 |
| | teamSop#5.7 | prsnSop#5.7.1〔prsn玩家·電子書內容頁〕段落 `Name:` 說話人依主題色上色、以面板篩選/於指定說話人暫停〔[runWi自訂Usr段落說話人上色]〕 |
...
* **電子書解析（增量1／增量2）**：`VersOne.Epub` NuGet（[techItem電子書解析]；增量1 以 `EpubReader.OpenBookAsync` 惰性載入書櫃清單、`CoverImage`／`ReadingOrder`／`Navigation`／`Schema.Package.Metadata.Identifiers`／`EpubReaderOptions` 容錯；**增量2 以 `EpubReader.ReadBookAsync` 取整本章節內容供內容頁段落渲染**〔`ExtractParagraphs` 剝 XHTML 取內文純文字，段落 `<p>` 與清單項 `<li>` 依文件位置合流〕；netstandard2.0 零相依、.NET 9 相容、純受管、無外部行程/服務）。
* **電子書逐段朗讀（增量2）**：`System.Speech.Synthesis`（SAPI，[techItem語音合成]、離線免金鑰）——`SpeechService`／`ISpeechService` 增**朗讀完成回呼**（`SpeakCompleted`）以驅動逐段自動前進，沿用既有語音選擇與 `SpeechRateSettings` 語速；無語音包明確降級不當機。
...
| 62 | runWi自訂Usr逐段導讀閱讀 | 58 | 電子書分頁【內容】開一本已入櫃的書 → 左章節目錄列各章（`Navigation` 葉章/spine 序）、中閱讀區以 `ExtractParagraphs` 剝章節 XHTML（`ReadBookAsync`）取 `<p>` 段落純文字逐段呈現、當前段放大高亮；`下一段`／`上一段`／`繼續` → 當前段游標前後移動（line-stepped、`PauseDecider` 段序前進，注入假段落 cue 可測）；雙擊某章 → 跳讀該章第一段、雙擊右側逐段清單某段 → 跳讀該段；讀到某章/段關程式重開 → 回上次閱讀位置（`EbookStore.SetReadingProgress`/`GetReadingProgress` 之 `LastReadChapter`/`LastReadParagraph` 還原）；EPUB 內嵌圖/複雜排版之章 → 以純文字段落呈現不當機；原檔唯讀不改寫（`ExtractParagraphs`／段序前進／進度換置純函式測涵蓋）|
| 63 | runWi自訂Usr逐段朗讀 | 62 | 閱讀中按朗讀鈕 → Windows 內建語音（SAPI，`SpeechService`）唸當前段呼叫發生（測試攔截、無網/無金鑰亦可）、唸完 `SpeakCompleted` 驅動游標前進下一段續唸（連續導讀）；`續讀`／`重聽本段` 正確、`語速`下拉調整（沿用 `SpeechRateSettings`）即時生效；切書/跳讀/暫停 → `SpeakAsyncCancelAll` 止、不誤讀；朗讀游標與當前段高亮同步；缺語音包 → 明確降級不當機（`SpeakCompleted` 自動前進接線以假 `ISpeechService` 注入驗）|
| 64 | runWi自訂Usr閱讀查詞加筆記 | 62 | 段落逐字可點（沿用 `EnglishWordTokenizer` 切詞、比照影片 `RenderClickable`）→ 點某英文單字以既有 `QueryService` 查得三欄並可朗讀（來源改內文文字、含標點之詞剝標點取原詞）、結果於 Dictionary 分頁呈現；「加入我的筆記」→ 經既有 `NotesStore` 去重加入指定夾、toast、於筆記分頁可見並可發音練習；與螢幕/影片來源筆記無差別共存；右鍵複製段落原文；查詢降級/重試沿用既有（測試攔截、不打真網路）|
| 65 | runWi自訂Usr段落說話人上色 | 62 | 開含 `Name:` 前綴之對話書 → 段首說話人以既有行首抽取（比照 `ExtractInlineSpeakers`、合唸 `Peppa/Suzy` 交 `PauseDecider.SplitSpeakers` 拆原子 Peppa/Suzy）填入、無前綴段不標；依使用中主題 12 色（描述含該說話人名、比照 `RebuildSpeakerColors`/`_speakerColorHex`）套段落字型色、切主題下拉 → 重算配色；右側說話人面板篩選（只看某些說話人）、於指定說話人暫停導讀（沿用 `PauseMatchesSet`/`SpeakerMatches`）→ 導讀前進停在符合者、跳過其餘；免 AI（段首說話人抽取/拆原子/配色判定/暫停符合為純函式測涵蓋）|
| 66 | runWi自訂Usr逐段導讀閱讀（段落與清單項混排） | 62 | 開一本**旁白 `<p>` ＋對白 `<ul class="exchange"><li>` 混排**之書（真實 EPUB 樣態）→ 章內 `<p>`、`<h1>`–`<h6>` 與 `<li>` **全數成段且依原文順序交錯**（不再因章內有 `<p>` 而丟棄全部清單項）；`<li>` 之對白文字完整、段數＝各區塊數；既有「章內無 `<p>`」之區塊層退回行為不迴歸（`<div>` 仍僅於無 `<p>` 亦無 `<li>` 時納入、與 `<p>` 併存時不重複計段）|
| 67 | runWi自訂Usr段落說話人上色（語意標記優先） | 66 | 對白為 `<li><strong class="speaker">Faculty:</strong> …</li>` 者 → 直接取語意標記為 `Speaker`（`Faculty`）、`Text` 不含該前綴，且**不受行首正則之 `≤3` 詞/`≤24` 字與「冒號後須空白」邊界限制**；無語意標記之段仍走既有段首 `Name:` 抽取（不迴歸）；[modVideoCapture模組] `SubtitleParser.ExtractInlineSpeakers` **未被改動**（影片路徑測項全綠） |
| 68 | runWi自訂Usr逐段導讀閱讀（導覽清單不成段） | 66 | 開目錄／導覽章（`<ol class="programme-list"><li><a href="…">章名</a></li>`）→ 純導覽清單項**不成段、不朗讀**；同章之非導覽 `<li>`（含連結外另有文字者）仍成段 |
| 69 | runWi自訂Usr逐段導讀閱讀（巢狀清單不重複） | 66 | 章內含 `<li>` 巢狀 `<p>`、以及 `<ul>` 巢狀 `<ul>` → 只取內層、同一段文字**不重複計段**；段序穩定（同書同輸出） |
| 70 | runWi自訂Usr逐段導讀閱讀（場景圖分隔線拖曳） | 62 | 開一本**含內嵌圖**之書 → 中欄自上而下＝場景圖塊／分隔線／「顯示：」篩選列／閱讀區；**往上拖分隔線** → 場景圖塊變矮、**閱讀區同步變高**（兩者互為消長、高度總和不變），「顯示：」篩選列高度**不變**、圖與篩選列之間**不出現空白帶**；**往下拖** → 反向消長且閱讀區不低於下限（不被壓成 0）；拖拉後切至他章再切回 → 圖塊維持拖拉後高度（不跳回預設）；另開一本**純文字書** → 場景圖塊與分隔線整塊收起、篩選列緊貼頁首（不留殘高）。（版面高度消長以修前／修後實機截圖對照存證；`SetupSceneImageBlock` 之有圖／無圖分支與「保留使用者拖拉後高度」判定為既有行為、不得回歸） |
| 71 | runWi自訂Usr逐段導讀閱讀（章界雙擊換章） | 62 | 【內容】頁讀至**章末**（`↓` 已無下一段可走）→ 於 `400ms` 內連按兩下 `↓` → **跳至下一章開頭**；改按兩下 `Space` 同效；讀至**章首**連按兩下 `↑` → 回**上一章**開頭。**章中**（非章界）連按兩下 `↓`／`Space`／`↑` → 維持既有單擊語意（下一段／播放·繼續／上一段）、**不換章**；兩下間隔**超過門檻**或**中間換了別的鍵** → 不成雙擊。換章後緊接第三下 → **不再連跳第二章**（雙擊狀態已重置）。末章章末雙擊／首章章首雙擊 → no-op、不當機。判定 `ChapterHopDecider` 為純函式（不讀時鐘、間隔由呼叫端傳入），上列分支以注入間隔與游標之單元測試逐條涵蓋；實機接線另以桌面 UIA 驗「章末雙擊確實換章、章中雙擊不換章」 |
