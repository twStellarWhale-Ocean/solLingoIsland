...
| 71 | runWi自訂Usr逐段導讀閱讀（章界雙擊換章） | 62 | 【內容】頁讀至**章末**（`↓` 已無下一段可走）→ 於 `400ms` 內連按兩下 `↓` → **跳至下一章開頭**；改按兩下 `Space` 同效；讀至**章首**連按兩下 `↑` → 回**上一章**開頭。**章中**（非章界）連按兩下 `↓`／`Space`／`↑` → 維持既有單擊語意（下一段／播放·繼續／上一段）、**不換章**；兩下間隔**超過門檻**或**中間換了別的鍵** → 不成雙擊。**自倒數第二段連按兩下 `↓`** → 只走到章末、**不跳章**（末段不被跳過），再按一下才換章。換章後緊接第三下 → **不再連跳第二章**（雙擊狀態已重置）。末章章末雙擊／首章章首雙擊 → no-op、不當機。判定 `ChapterHopDecider` 為純函式（不讀時鐘、間隔由呼叫端傳入），上列分支以注入間隔與游標之單元測試逐條涵蓋；實機接線另以桌面 UIA 驗「章末雙擊確實換章、章中雙擊不換章」 |
