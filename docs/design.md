...
> orgSop 數＝需求域 4 條（螢幕／影片／筆記／電子書）＋平台維保 1 條＝5，與 spec 12 條不等值（電子書一域 orgSop#0.4 跨兩增量對應七條 spec；維保原無獨立 value-spec，**惟 spec#11 為例外**——其由 orgSop#0.1 與 orgSop#1.1 共同承接，見 ＜III 規格指標＞ 前言）。
