...
- **（v4.0.0 規劃中·plan 意圖稿）電子書內容閱讀**：把書櫃裡的書**翻開來讀**——**逐段導讀**（上一段／下一段／繼續、雙擊章節或段落跳讀、讀到哪記到哪）、聽 **TTS 朗讀**（唸完自動接下一段）、**點字查詞**並加進「我的筆記」、對白**依說話人上色**；沿用你熟悉的查詢／筆記／朗讀那套，不必另學。此為設計階段意圖稿、待實作截圖校準（詳見下方「電子書內容閱讀」一節）
...
| 開啟主視窗 | 點工作列 LingoIsland 按鈕／`Alt+Tab`，或系統匣圖示 → 開啟**統一主視窗**（上方分頁：**主題**／**螢幕截圖**／**影片**／**電子書**／筆記／歷史／選項／關於；Dictionary 為右上獨立視窗鈕） |
| 電子書書櫃 | 「電子書」分頁 →【獲得】：**左＝書櫃**（封面縮圖卡／依主題篩選／依加入時間·書名·作者·主題排序／右鍵或 `Delete` 刪、清空）｜**右＝匯入卡片**（選檔或拖入 `.epub`、可批次、彙總確認匯入；同一本書依 dc:identifier 去重）。翻開閱讀／朗讀／查詞為後續版本 |
...
![電子書分頁【獲得】子頁（實機）：左＝書櫃三本樣書封面縮圖卡（主題篩選／四鍵排序／右鍵刪除／清空）｜右＝匯入卡片（選檔或拖入 .epub、批次彙總確認）](docs/manual-assets/ebook-bookshelf.png)
...
- **接下來（增量 2，規劃中）**：翻開書逐章閱讀、逐段導讀、朗讀、點字查詞、把字句加入「我的筆記」、依說話人上色——都沿用你已熟悉的那套查詢／筆記／發音練習。詳見下節「電子書內容閱讀」。

## 電子書內容閱讀（epic #228 / 增量 2，v4.0.0 規劃中）

> **本節為 plan 意圖稿（設計階段）**——描述「翻開書來讀」規劃如何運作，實際畫面與逐步操作**待實作後以真實截圖與說明校準**。下圖為**設計期參考稿**、非最終畫面。

把已經收進書櫃的書**翻開來讀、邊讀邊學**——這一版讓電子書從「收藏」進到「閱讀」：

1. 開 **電子書** 分頁 →【**內容**】子頁籤，從**左側章節目錄**點一本書、選一章。
2. 內文以**一段一段**呈現，**目前這段會放大highlight**；用底下的 **上一段／下一段／繼續** 逐段往下讀，或**雙擊左邊某一章、或右邊逐段清單裡某一段**直接跳過去讀。**讀到哪一章、哪一段會記住**，關掉程式下次打開接著讀。
...
![電子書內容頁（閱讀器）設計期參考稿·待 code 以實際截圖替換：左＝章節目錄｜中＝閱讀區（當前段放大高亮、逐字可點查詞、對白依說話人上色）＋控制列（上一段／下一段／繼續／朗讀／語速／主題）｜右＝逐段清單＋說話人面板](docs/design-visual/page-ebook-reader.svg)
> 上圖為**設計稿（plan 意圖稿）**，實際外觀與操作以實作後之真實截圖為準。閱讀渲染本版先做**段落純文字**，EPUB 內嵌的圖片與複雜排版留待後續版本。
