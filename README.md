...
5. **查看結果**：結果顯示於**獨立的「字典」視窗**（原文／KK 音標／中譯）；點 `▶ Play` 聽整句，或**點英文句中任一單字查該字字義**（往前鈕返回原句），辨識有錯可按**鉛筆鈕**改原文重譯；字典視窗會浮在無邊框遊戲之上、**與主視窗共存互不干擾**，`Alt+Tab` 回遊戲即可。也可直接在字典視窗頂部**打字查詢**（或按下拉挑選查詢歷史）。
...
| 整句發音 | 字典視窗 `▶ Play` 按鈕（重按會重新播放） |
| 單字查詢 | 點選英文原句中的任一單字，即**查詢該單字字義**（原文/音標/中譯；查詢中游標轉圈、避免連點）；用頂部**往前／往後**鈕可返回原句，單字查詢結果不會自動加入筆記 |
| 編輯重新翻譯 | 辨識錯字時，字典視窗頂部**鉛筆鈕**→改英文原文→「Re-translate」自動重譯；筆記／歷史條目**右鍵「Edit text」**亦可就地校正原文、自動更新中文 |
| 開啟主視窗 | 點工作列 LingoIsland 按鈕／`Alt+Tab`，或系統匣圖示 → 開啟**統一主視窗**（上方分頁依序：**主題**／**擷取**／**影片**／**電子書**／筆記／歷史／選項／關於；功能列**右端「字典」鈕**不是分頁，按了會喚出獨立的字典視窗） |
...
| 查看查詢結果 | 結果在**獨立的「字典」視窗**——收起來以後，按主視窗功能列**右端「字典」鈕**（或系統匣右鍵選單「Result」）就能再喚出來：已開著＝帶到前景／已收起且有查詢歷史＝以最新一筆重新開卡／完全沒有歷史＝右下角提示「No query result yet」 |
| 查詢歷史 | 主視窗「歷史」分頁：左側選日期 → 右側該日紀錄（新在上）；**點一下即選取**（卡片深粉框標示）；每筆可 `＋筆記`、`▶ 播音`、`檢視`、`刪除`；頂部「清除全部」 |
| 加入我的筆記 | 字典視窗**底部**「＋ 加入我的筆記」，或歷史條目「＋筆記」→ 右下角閃示「已加入」（已收藏過則提示「已在筆記中」） |
...
> **v1.0.0 → v1.0.1（現行）**：v1.0.0 曾把查詢結果併入主視窗的 Dictionary 分頁；**v1.0.1 已改回獨立的「字典」視窗**——因為在筆記頁檢視單字時，主視窗會整個跳到 Dictionary 分頁、打斷正在進行的發音練習。現行字典視窗可直接打字查詢＋下拉挑選查詢歷史，會記住你擺放的位置與大小，`ESC`／「✕」＝收起來而非關掉。上述 v0.31–0.32 的舊截圖已不適用，待補現行字典視窗新截圖。
...
![字典結果底部「底色」列——十色一列全可視（v0.35.0）](docs/manual-assets/result-swatches-ten.png)
...
4. **書櫃**裡每本是一張**封面縮圖卡**——由上而下＝**主題／書名／作者／章數**（v4.8.0 起主題移到最上；**書名太長會自動換行完整顯示、不會被切掉**，同系列書才分得出是哪一本）；上方可**依主題篩選**、**依加入時間／書名／作者／主題排序**（點一下切遞增／遞減、再點翻向；選擇會記住）；想丟掉哪本就在它上面**右鍵「Delete」或按 `Delete` 鍵**，也可以一次「清空書櫃」。
5. 收進來的書**長期保存在你本機**（每本一個資料夾，含書名／作者／目錄等資料、原始 `.epub` 複本與封面）；關掉重開書櫃原樣還在，換電腦時跟著「選項 → 資料備份與搬遷」的匯出／匯入一起帶走。

![電子書分頁（實機）：最左＝常駐書目清單（三本樣書封面縮圖卡、主題篩選／四鍵排序／右鍵刪除／清空）｜右側【獲得】子頁＝匯入卡片（選檔或拖入 .epub、批次彙總確認；v4.5.2 頁面整併）](docs/manual-assets/ebook-bookshelf.png)
...
![書櫃卡片版式（實機，v4.8.0）：每張卡由上而下＝主題／書名／作者／章數；四本同系列書之書名皆完整換行顯示，Level 1–4 一眼可辨](docs/manual-assets/shelf-card-booklist.png)
...
2. 內文以**一段一段**呈現，**目前這段會放大highlight**；用底下的 **上一段／下一段／繼續** 逐段往下讀，或**點右側某一章、點閱讀區裡某一段**直接跳過去讀（段落上按右鍵可複製整段、加入筆記）。**讀到哪一章、哪一段會記住**，關掉程式下次打開接著讀。場景圖下方的「**顯示**」下拉可調說話人呈現：全部顯示／只顯示勾選角色的段落（章節標題與目前這段仍會顯示）／只加粗／只上色。
3. 想用聽的——按**朗讀**鈕，用 Windows 內建語音把**這一段**唸出來，**唸完自動接下一段**（像有聲書一路讀下去）；可以**續讀／重聽這段／調語速**（沿用你原本的朗讀語音與語速設定）。**（v4.9.0 起）中英文都會唸**：同一段裡中英夾雜也照唸，英文段用英文語音、中文段用中文語音，一句之內自動換聲；系統若還沒裝中文語音，中文段會以預設語音念並提示你到「設定 → 語言 → 語音」加裝。
...
![電子書內容頁（閱讀器·實機）：中＝場景圖＋顯示篩選＋閱讀區（當前段放大高亮、逐字可點查詞、對白依說話人上色）＋控制列（上一段／下一段／繼續／朗讀／語速／主題）｜右＝章節清單＋說話人面板（v4.5.1 版面整理）](docs/manual-assets/ebook-reader.png)
> 上圖為**實機截圖**：翻開自製對白樣書「A Rainy Day」逐段導讀，對白依說話人（Anna／Ben／Mum／Dad）上色。閱讀渲染本版先做**段落純文字**，EPUB 內嵌的圖片與複雜排版留待後續版本。

![中英混排段落之逐段朗讀（實機，v4.9.0）：`Anna: Good morning 早安, Ben.` 唸完後自動前進到下一段 `Ben: 我很好, thank you very much.`](docs/manual-assets/ebook-bilingual-speak.png)
...
![電子書內容頁·拖曳場景圖分隔線（實機）：往上拉後場景圖變矮、內文區同步變高，「顯示：」篩選列緊貼圖下不留空白（v4.6.2）](docs/manual-assets/ebook-splitter-drag.png)
...
現在**朗讀中閱讀區的游標會變成「忙碌但仍可操作」的樣式**（箭頭旁多一個忙碌指示），唸完、暫停、跳段、切書、切章就回復正常。

游標**不是**那種「沙漏／轉圈」的等待樣式——那代表畫面卡住不能動。朗讀中你**照樣**可以逐字點選查詞、跳段、暫停、切章，所以用的是「程式在動，但你隨時可以插手」那一款。

左邊章節目錄和右邊說話人面板的游標不受影響；段落上的單字仍然是「可點」的手指游標。

## 改了配色，其他頁馬上跟著變

**v4.11.0 起。** 以前在「主題」分頁改了 12 格顏色或顏色說明、按了儲存，**切到電子書或影片頁，說話人的顏色還是舊的**——得重開程式才會更新。
...
![主題頁改了沒存就切走時跳出的提示——三選：儲存並離開／捨棄變更並離開／取消留在主題頁](docs/manual-assets/theme-unsaved-guard.png)
...
> **範圍**：目前會這樣提醒的是**主題分頁**和**選項分頁**。其他分頁還沒有——例如影片頁的「Edit YAML」也是可以編輯的地方，這一版不會提醒你。
>
...
> **「選項」分頁新增兩區＋守衛**：**條目顯示**（條目字級／粗體／自動換行／**過關卡是否透明底**）、**字典結果**（基準字級）；另外在選項頁改了設定**沒按「儲存」就切到別的分頁**時，會**三選提示**：**是＝儲存並離開／否＝捨棄變更並離開／取消＝留在原頁**（選「是」會先存檔、成功才離開，存不起來就不離開、留在頁上告訴你原因）；而**按「儲存」成功時改為在底部狀態列輕量閃示「Saved ✓」、不再彈出對話框**。（v4.10.0 起**關掉程式時也會先問你**，見上節「主題頁／選項頁改了沒存，切走前會先問你」。）
...
![選項分頁「筆記／歷史條目」（條目字級／粗體／換行）與「字典結果」（結果基準字級）兩區；失焦自動隱藏自 v1.0.0 移除後未再恢復（v1.0.1 改回獨立字典視窗時亦未一併恢復）](docs/manual-assets/options-entry-window-settings.png)
