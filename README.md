...
**v4.10.0 起。** 在**主題**分頁改了東西、**沒按「儲存」就切走，改的內容以前會直接不見、連問都不問**。現在會先跳提示，畫面上的按鈕是「是／否／取消」：

- **是＝儲存並離開** —— 存不起來就**不會離開**，會留在頁上告訴你原因。
- **否＝捨棄變更並離開** —— 把這次的改動丟掉，回到上次儲存的內容。
- **取消＝留在原處** —— 你改到一半的內容**原封不動**，分頁也不會跳走。按 `ESC` 或關掉這個提示，也等於選「取消」。
