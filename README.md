...
- **（v3.15.0 新增）電子書擷取**：把你自己的 **EPUB 電子書**加入**書櫃**——選檔或拖進 `.epub`（可一次多本批次），程式讀出書名、作者與章節目錄、擺上有**封面縮圖**的書櫃，可**依主題整理、依加入時間／書名／作者／主題排序**，同一本書不會重複收；藏書長期保存在你本機、換電腦可隨資料備份一起帶走。翻開內文閱讀、逐段導讀與朗讀為**後續版本**（本階段先把「加入書櫃、整理保存」做好）
